...
import random
import numpy as np
# @param ["Single cell spikes", "Intercellular transients", "Intercellular waves", "Fluttering"]
...
class Pouch(object):
    def __init__(self, params=None, size='xsmall', sim_number=0, save=False, saveName='default'):
        ...
        disc_vertices = np.load(
            # Vertices
            "./geometry/disc_vertices.npy", allow_pickle=True).item()
        disc_laplacians = np.load(
            # Laplacian Matrix
            "./geometry/disc_sizes_laplacian.npy", allow_pickle=True).item()
        disc_adjs = np.load("./geometry/disc_sizes_adj.npy",
                            allow_pickle=True).item()  # Adjacency matrix
        ...
        # Simulation to run for 3600 seconds (1 hour)
        ...
        # Lower bound of standby cell VPLCs
        self.lower = self.param_dict['lower']
        # Upper bound of standy cell VPLCs
        self.upper = self.param_dict['upper']
        self.frac = self.param_dict['frac']   # Fraction of initiator cells

        # Initialize disc_dynamics to save simulation calcium, IP3, calcium_ER, ratio
        self.disc_dynamics = np.zeros((self.n_cells, 4, self.T))
        # Initialize VPLC array for cells
        self.VPLC_state = np.zeros((self.n_cells, 1))

    def simulate(self):  # Simulate dynamics of system
        # Set the seed for reproducibility (keep initiator cells consistent each run)
        np.random.seed(self.sim_number)

        # Initialize simulation ER Calcium
        self.disc_dynamics[:, 2, 0] = (
            self.c_tot-self.disc_dynamics[:, 0, 0])/self.beta
        # Initialize simulation fraction of inactivated IP3R receptors
        self.disc_dynamics[:, 3, 0] = np.random.uniform(.5, .7, size=(
            self.n_cells, 1)).T
        # Initialize the values for VPLCs of standby cells to be random uniformly distributed from lower to upper
        self.VPLC_state = np.random.uniform(
            self.lower, self.upper, (self.n_cells, 1))
        stimulated_cell_idxs = np.random.choice(self.n_cells, int(
            self.frac*self.n_cells))  # Choose which cells are initiator cells
        # Set the VPLC of initiator cells to be random uniformly distributed between 1.3 and 1.5
        self.VPLC_state[stimulated_cell_idxs, 0] = np.random.uniform(
            1.3, 1.5, len(stimulated_cell_idxs))

        # Establish the VPLCs to be passed into the ODE approximations
        ...
        for step in range(1, self.T):
            ...
            # ODE EQUATIONS
            self.disc_dynamics[:, 0, step] = (ca+self.dt*(ca_laplacian+(self.k_1*(np.divide(np.divide(r*np.multiply(
                ca, ipt), (self.k_a+ca)), (self.k_p+ipt)))**3 + self.k_2)*(s-ca)-self.V_SERCA*(ca**2)/(ca**2+self.K_SERCA**2))).T
            self.disc_dynamics[:, 1, step] = (ipt+self.dt*(ipt_laplacian+np.multiply(
                V_PLC, np.divide(ca**2, (ca**2+self.K_PLC**2)))-self.K_5*ipt)).T
            self.disc_dynamics[:, 2, step] = ((self.c_tot-ca)/self.beta).T
            self.disc_dynamics[:, 3, step] = (
                r+self.dt*((self.k_tau**4+ca**4)/(self.tau_max*self.k_tau**4))*((1-r*(self.k_i+ca)/self.k_i))).T

    def make_animation(self, path=None):  # Creation of calcium video
        colormap = plt.cm.Greens
        normalize = matplotlib.colors.Normalize(vmin=np.min(
            self.disc_dynamics[:, 0, :]), vmax=max(np.max(self.disc_dynamics[:, 0, :]), 1))
        with sns.axes_style("white"):
            ...
            cbar.ax.set_yticklabels(
                cbar.ax.get_yticklabels(), fontsize=15, fontweight="bold")
            for cell in self.new_vertices:
                ax.plot(cell[:, 0], cell[:, 1],
                        linewidth=0.0, color='w', alpha=0.0)
            patches = [matplotlib.patches.Polygon(
                verts) for verts in self.new_vertices]
            ...
            def animate(frame, time_stamps):
                for j in range(len(patches)):
                    c = colors.to_hex(
                        colormap(normalize(frame[j])), keep_alpha=False)
            ...
            anim = animation.FuncAnimation(fig, animate,
                                           init_func=init,
                                           # Disc dynamics: Array of: [n cells x 4 x time-samples]
                                           frames=self.disc_dynamics[:,
                                                                     0, ::50].T,
                                           fargs=(time_stamps,),
                                           interval=70,
                                           blit=True)
        if self.save:
            if path != None:
                ...
                # Used to save specifically the demo videos
                anim.save(path+"/"+'demo_video.mp4')
            else:
    ...
    def draw_profile(self, path=None):  # Draw the VPLC Profile for the simulation
        ...
        with sns.axes_style("white"):
            ...
            cbar = fig.colorbar(sm, ax=ax)
            cbar.ax.set_yticklabels(
                cbar.ax.get_yticklabels(), fontsize=80, fontweight="bold")
            for cell in self.new_vertices:
                ax.plot(cell[:, 0], cell[:, 1], linewidth=1.0, color='k')
            for k in range(len(self.new_vertices)):
                cell = self.new_vertices[k]
                c = colors.to_hex(colormap(normalize(self.VPLC_state[k]))[
                                  0], keep_alpha=False)
        ...
        if self.save:
            if path != None:
                if not os.path.exists(path):
                    os.makedirs(path)
                fig.savefig(path+"/"+self.size+'Disc_VPLCProfile_'+str(self.sim_number) +
                            '_'+self.saveName+".png", transparent=True, bbox_inches="tight")
            else:
    ...
    def draw_kymograph(self, path=None):  # Draw the calcium Kymograph for the simulation
        with sns.axes_style("white"):
            ...
            y_axis = centeriods[:, 1]
            # Location of where to draw the kymograph line
            kymograp_index = np.where((y_axis < (-490)) & (y_axis > (-510)))

            colormap = plt.cm.Greens
            normalize = matplotlib.colors.Normalize(vmin=np.min(
                self.disc_dynamics[:, 0, :]), vmax=max(1, np.max(self.disc_dynamics[:, 0, :])))
            ...
            ax = plt.gca()
            plt.yticks(np.arange(0, self.T/2, 1498),
                       [0, 10, 20, 30, 40, 50, 60], fontsize=30, fontweight="bold")
            plt.xticks([])
            ...
            if self.save:
                if path != None:
                    if not os.path.exists(path):
                        os.makedirs(path)
                    fig.savefig(path+"/"+self.size+'Disc_Kymograph_'+str(self.sim_number) +
                                '_'+self.saveName+".png", transparent=True, bbox_inches="tight")
                else:
                    print("Provide a path for saving images")
    ...
    def getSignalCells(self, threshold=0.06, whichSignal='Calcium', wantSignal=True):
        ...
        # Rename the dataframe column to "VPLC"
        VPLCdf.rename(columns={VPLCdf.columns[-1]: "VPLC"}, inplace=True)
        temp = pd.concat(
            [pd.DataFrame(self.disc_dynamics[:, selector, :]), VPLCdf], axis=1)
        signalsIndex = []
        nonSignalsIndex = []
    ...
    def plotFWHM(self, plotLim, peakWidthDist=100, typeActivity='Spike', whichSignal='Calcium', baselineThreshold='auto', getStats=True, saveFig=False, path='./testResults/cellSizeControl', outputPlot=True):
        ...
        # Signaling Cells. 1:-1 selects time steps but not the VPLC value
        signaling = self.getSignalCells(
            threshold=baselineThreshold, whichSignal=whichSignal).iloc[:, 1:-1]
        # Non-Spiking Cells. 1:-1 selects time steps but not the VPLC value
        nonSignaling = self.getSignalCells(
            threshold=baselineThreshold, whichSignal=whichSignal, wantSignal=False).iloc[:, 1:-1]
        ...
        peakIndexes = list(find_peaks(
            concat['signal'][0:], distance=peakWidthDist))[0]
        ...
        getPeakWidths = peak_widths(
            concat['signal'][0:], peakIndexes, rel_height=0.5)
        ...
        for i in range(0, nPeaks-1):
            period.append(peaksPlot['Time (s)'].iloc[i+1] -
                          peaksPlot['Time (s)'].iloc[i])
        freq = 1/pd.DataFrame(period).median()[0]
        fullFreq = 1/pd.DataFrame(period)

        integratedInt = simps(concat['signal'], dx=self.dt)

        stdErrMedPeak = 1.96 * 1.2533 * \
            peaksPlot['signal'].std() / np.sqrt(len(peaksPlot['signal']))
        stdErrMedFWHM = 1.96 * 1.2533 * \
            pd.DataFrame(FWHM).std() / np.sqrt(len(pd.DataFrame(FWHM)))
        ...
        if outputPlot:
            ...
            ax.legend(['Signal Cells', 'Non-signal Cells'],
                      loc=2, bbox_to_anchor=(1.1, 0.8))
        ...
        if saveFig:
            if path != None:
                if not os.path.exists(path):
                    os.makedirs(path)
                ax.get_figure().savefig(path+"/"+typeActivity+"_"+whichSignal+"_" +
                                        str(self.n_cells)+"Cells"+"_FWHM"+".pdf", transparent=True, bbox_inches="tight")
            else:
                print("Provide a path for saving images")
    ...
    def plotAllDynamics(self, typeActivity='Spike', baselineThreshold='auto', saveFig=False, path='./testResults/cellSizeControl', nameAppend='allDynamics'):
        ...
        if baselineThreshold == 'auto':
            baselineThreshold = [self.getBaselineSignal(
                whichSignal='Calcium'), self.getBaselineSignal(whichSignal='IP3'), 0.0, 0.0]
        else:
            baselineThreshold = baselineThreshold
        ...
        for i in range(0, len(signals)):
            signaling[signals[i]] = self.getSignalCells(
                baselineThreshold[i], signals[i])  # Signaling Cells
            nonSignaling[signals[i]] = self.getSignalCells(
                # Non-Spiking Cells
                baselineThreshold[i], signals[i], wantSignal=False)
            # Plotting dataframes. 2:-1 selects time steps but not the VPLC value
            a[signals[i]] = pd.DataFrame(
                signaling[signals[i]].iloc[:, 25:-1].mean().transpose())
            a[signals[i]] = a[signals[i]].rename(
                columns={0: 'signal'+signals[i]})
            # Plotting dataframes. 2:-1 selects time steps but not the VPLC value
            b[signals[i]] = pd.DataFrame(
                nonSignaling[signals[i]].iloc[:, 25:-1].mean().transpose())
            b[signals[i]] = b[signals[i]].rename(
                columns={0: 'nosignal'+signals[i]})
        ...
        p1, = host.plot(concat['Time (s)'], a['Calcium'],
                        "b-", label="Calcium Intensity (a.u.)")
        p2, = par1.plot(concat['Time (s)'], a['IP3'],
                        "r-", label="IP3 Intensity (a.u.)")
        p3, = par2.plot(concat['Time (s)'], a['ER Calcium'],
                        "g-", label="ER Calcium Intensity (a.u.)")
        p4, = par3.plot(concat['Time (s)'], a['IP3R'],
                        "m-", label="Fraction of inactive IP3 Receptors")
        ...
        host.legend(lines, [l.get_label()
                    for l in lines], loc=2, bbox_to_anchor=(1.6, 0.8))
        ...
        if saveFig:
            if path != None:
                if not os.path.exists(path):
                    os.makedirs(path)
                host.get_figure().savefig(path+"/"+typeActivity+"_"+str(self.n_cells)+"Cells" +
                                          "_"+nameAppend+".pdf", transparent=True, bbox_inches="tight")
            else:
                print("Provide a path for saving images")
    ...
    def signalHeatMap(self, whichSignal='Calcium', lowerLim='auto', upperLim='auto', path=None, saveFig=False, hideFig=True, typeActivity='Spike'):
        ...
        summedDynamics = pd.DataFrame(
            self.disc_dynamics[:, selector, :]).sum(axis=1)
        ...
        with sns.axes_style("white"):
            ...
            for k in range(len(self.new_vertices)):
                cell = self.new_vertices[k]
                c = colors.to_hex(
                    colormap(normalize(summedDynamics.iloc[k])), keep_alpha=False)
                ax.fill(cell[:, 0], cell[:, 1], c)
        ...
        if saveFig:
            if path != None:
                if not os.path.exists(path):
                    os.makedirs(path)
                plt.savefig(path+"/"+whichSignal+"_"+str(self.n_cells)+"Cells"+"_" +
                            "heatMap"+"_"+typeActivity+".png", transparent=True, bbox_inches="tight")
            else:
                print("Provide a path for saving images")
        if hideFig:
            plt.close(fig)
...
# The folder where simulation results are to be saved for this notebook
saveFolderName = './simulationResults/Pouch_Visualization'
# Create the folder if it does not yet exist
if not os.path.exists(saveFolderName):
    os.makedirs(saveFolderName)

p_Simulation = Pouch(params=sim_params, size='large',
                     sim_number=12345, save=True, saveName='Demo_Simulation')
...
mp4 = open(
    '/content/simulationResults/Pouch_Visualization/demo_video.mp4', 'rb').read()
...
# The folder where simulation results are to be saved for this notebook
saveFolderName = './simulationResults/Additional_Plots'
# Create the folder if it does not yet exist
if not os.path.exists(saveFolderName):
    os.makedirs(saveFolderName)

for i in range(0, len(pouchesList)):
    pouchesList[i].plotAllDynamics(typeActivity=save_Name, baselineThreshold='auto',
                                   saveFig=True, nameAppend="AllDynamics", path=saveFolderName)
...
for i in range(0, len(pouchesList)):
    pouchesList[i].signalHeatMap(whichSignal='Calcium', lowerLim=0, upperLim='auto',
                                 path=saveFolderName, saveFig=True, hideFig=False, typeActivity=save_Name)

for i in range(0, len(pouchesList)):
    pouchesList[i].signalHeatMap(whichSignal='IP3', lowerLim=0, upperLim='auto',
                                 path=saveFolderName, saveFig=True, hideFig=False, typeActivity=save_Name)

for i in range(0, len(pouchesList)):
    pouchesList[i].signalHeatMap(whichSignal='ER Calcium', lowerLim='auto', upperLim='auto',
                                 path=saveFolderName, saveFig=True, hideFig=False, typeActivity=save_Name)

for i in range(0, len(pouchesList)):
    pouchesList[i].signalHeatMap(whichSignal='IP3R', lowerLim='auto', upperLim='auto',
                                 path=saveFolderName, saveFig=True, hideFig=False, typeActivity=save_Name)
